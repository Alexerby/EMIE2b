...
recursive_model_forecast <- function(hmax) {
...
rolling_model_forecast <- function(hmax) {

  # Create matrix for our ARIMA models
  ro_arima_mean <- ts(data = matrix(NA, nrow = n_origins, ncol = hmax),
          start = origin_1,
          frequency = frequency(cpif)
  )

  # Create matrix for our AR(1) models
  ro_ar1_mean <- ts(data = matrix(NA, nrow = n_origins, ncol = hmax),
            start = origin_1,
            frequency = frequency(cpif)
  )
  ...
  for (i in 1:n_origins) {
    ...
    start <- start(cpif) + (i - 1) / 12

    origin <- origin_1 + (i - 1) / 12 # calculates origin monthly (1/12)

    # here we provide start date, which we do not in recursive
    dat <- window(cpif, start = start, end = origin)
    ...
    # Fill matrix with mean
    ro_arima_mean[i, ] <- forecast_arima$mean

     # Append models to our respective lists
    ro_arima_models[[i]] <- model_arima
    ro_arima_forecasts[[i]] <- forecast_arima
    ...
    # Append the mean value to matrix
    ro_ar1_mean[i, ] <- forecast_ar1$mean

    # Append models to our respective lists
    ro_ar1_models[[i]] <- model_ar1
    ro_ar1_forecasts[[i]] <- forecast_ar1
  ...
  # (creating and using function at same time, 
  # here applying it at our re_forecasts list)
  ro_arima_list <- lapply(ro_arima_forecasts, function(x) x[["mean"]])
  ro_arima_plot <- do.call(ts.union, ro_arima_list)

  # Same for AR(1) models
  ro_ar1_list <- lapply(ro_ar1_forecasts, function(x) x[["mean"]])
  ro_ar1_plot <- do.call(ts.union, ro_ar1_list)

  return(list(
  ro_arima_plot = ro_arima_plot,
  ro_ar1_plot = ro_ar1_plot,
  ro_arima_mean = ro_arima_mean,
  ro_ar1_mean = ro_ar1_mean
  ))
}


# Store recursive models in variables
recursive_forecast_h1 <- recursive_model_forecast(1)
recursive_forecast_h12 <- recursive_model_forecast(12)
recursive_forecast_h24 <- recursive_model_forecast(24)

# Store rolling models in variables. The rolling
# models are necessary in order to answer question 4,
# when testing the forecast accuracy with
# Diebold-Mariano test.
rolling_forecast_h1 <- rolling_model_forecast(1)
rolling_forecast_h12 <- rolling_model_forecast(12)
rolling_forecast_h24 <- rolling_model_forecast(24)
